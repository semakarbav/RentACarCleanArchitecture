...
namespace Application.Features.Colors.Commands.CreateColor
{
    ...
    public class CreateColorCommandHandler : IRequestHandler<CreateColorCommand, Color>
    {
        ...
        {
            await _colorBusinessRules.CheckIfColorNameCanNotBeDuplicated(request.Name);
            var mapperedColor = _mapper.Map<Color>(request);
            var createdColor = await _colorRepository.AddAsync(mapperedColor);
            return createdColor;
        }
    }
}
